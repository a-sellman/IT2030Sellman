...
namespace Lab13.Models
{
    public class RouteDictionary : Dictionary<string, string>
    {
        public string EmployeeFilter
        {
            get
            {
                string s Get(nameof(SalesGrid.Employee));
                int index = s?.IndexOf('-') ?? -1;

                return index == -1 ? s : s.Substring(0, index);
            }
            set => this[nameof(SalesGrid.Employee)] = value;
        }
        ...
        {
            get => Get(nameof(SalesGrid.Quarter));
            set => this[nameof(SalesGrid.Quarter)] = value;
        }
        ...
        {
            get => Get(nameof(SalesGrid.Year));
            set => this[nameof(SalesGrid.Year)] = value;
        }

        public void ClearFilters() => EmployeeFilter = YearFilter = QuarterFilter = SalesGrid.DefaultFilter;
        ...
        private void SetSortAndDirection(string fieldName, RouteDictionary current)
        {
